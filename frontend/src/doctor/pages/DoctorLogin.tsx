...
const DoctorLogin: React.FC = () => {
  ...
  const {  loading, error } = useAppSelector(selectDoctorAuth);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resultAction = await dispatch(doctorLogin({ email, password })).unwrap();
  ...
  const inputClass = (name: string) => {
    const base = "w-full border rounded-lg p-2.5 text-sm shadow-md bg-white/90 backdrop-blur-sm transition";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-cover bg-center relative px-6" style={{ backgroundImage: "url('/assets/login-bg.jpg')" }}>
      ...
      <div className="relative z-10 w-full max-w-md bg-white/95 rounded-2xl shadow-2xl p-8 border border-white/20 backdrop-blur-xl animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <input name="email" type="email" placeholder="Email Address" value={email} onChange={handleChange} onBlur={handleBlur} className={inputClass("email")} required />
            {formErrors.email && <p className="text-red-500 text-xs mt-1">{formErrors.email}</p>}
          </div>

          <div className="relative">
            <input name="password" type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={handleChange} onBlur={handleBlur} className={`${inputClass("password")} pr-10`} required />
          ...
          <button type="submit" disabled={loading} className={`w-full py-2.5 rounded-lg text-white font-medium shadow-lg transition ${loading ? "bg-blue-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700"}`}>
